...
# DB Schema Audit — `init_schema()` vs `TECH_SPEC.md`

**Source of truth (target):** `file:TECH_SPEC.md` §5  
...
| `local_base_dir` | `TEXT NOT NULL DEFAULT 'generated/projects'` | |
...
- API surface still missing for:
  - `prompt_templates` CRUD
  - `asset_links` CRUD
  - `project_exports` read/list endpoints (canonical fields)
- Optional hardening/backfill:
  - full FK normalization from path-like fields (`final_output`, `output_path`, etc.) to asset IDs for all legacy rows

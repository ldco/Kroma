    ...
    R: PipelineCommandRunner,
{
    ...
    fn upscale_native(
        ...
        let script_path = self.app_root().join("scripts/realesrgan-python-upscale.py");
        if !script_path.is_file() {
            return Err(ToolAdapterError::Native(format!(
                "missing script: {}",
                path_for_output(self.app_root(), script_path.as_path())
            )));
        }
        let mut args = vec![
            script_path.to_string_lossy().to_string(),
...
mod tests {
    ...
    fn native_upscale_runs_python_backend_directly_with_config_defaults() {
        ...
        std::fs::write(
            app_root.join("scripts/realesrgan-python-upscale.py"),
            b"# test",
        )
        .expect("upscale script should exist");
        std::fs::create_dir_all(app_root.join("var/projects/demo/outputs"))
            .expect("outputs dir should exist");
        std::fs::create_dir_all(app_root.join("config")).expect("config dir should exist");
        ...
        assert!(seen[0]
            .args
            .iter()
            .any(|v| v.ends_with("scripts/realesrgan-python-upscale.py")));

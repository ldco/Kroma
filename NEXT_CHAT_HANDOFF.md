...
- `cargo test`
- `npm run backend:rust:test --silent`
7. Completed provider/style/character extraction into:
- `src-tauri/src/db/projects/provider_style_character.rs`
8. Wired `projects.rs` to re-export provider/style/character types and delegate schema/table/column setup to the new module.
9. Re-ran validation after extraction:
- `cargo fmt --all`
- `cargo test`
- `npm run backend:rust:test --silent`
